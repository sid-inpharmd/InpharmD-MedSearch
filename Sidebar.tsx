...
import { BookOpenText, Home, Search, SquarePen, Settings, ChevronRight, ChevronLeft, Paperclip } from 'lucide-react';
  ...
  return (
    <div
      className={cn(
        ...
        !isSidebarOpen ? 'items-center' : 'items-center'
      )}
    >
      {children}
    </div>
...
const Sidebar = ({ children }: { children: React.ReactNode }) => {
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      fileReader.onloadend = () => {
        localStorage.setItem('selectedFile', fileReader.result as string);
        router.push('/attach-file');
      };
  ...
  return (
    <div className="flex">
      ...
      <div
        className={cn(
          'relative fixed inset-y-0 z-50 flex flex-col justify-between overflow-y-auto bg-light-secondary dark:bg-dark-secondary px-2 py-8 transition-width duration-300',
...
>
  <img
    src="https://th.bing.com/th?id=OIP.R_rs-fJPEv830mc_8O0B_AAAAA&w=335&h=111&c=8&rs=1&qlt=90&o=6&dpr=1.3&pid=3.1&rm=2"
    alt="Logo"
    className="w-8 h-8"
  />
  {isSidebarOpen ? (
    <div className="flex items-center">
      <span className="ml-2 text-xl font-semibold sidebar-color">InpharmD</span>
    ...
    <SquarePen className="cursor-pointer text-6xl sidebar-color mt-6" />
        ...
        <div className="flex flex-col flex-grow justify-start mt-20">
          <VerticalIconContainer isSidebarOpen={isSidebarOpen}>
            ...
            <label
              htmlFor="file-input"
              className={cn(
                'relative flex items-center gap-x-4 cursor-pointer hover:bg-black/10 dark:hover:bg-white/10 duration-150 transition w-full py-2 rounded-lg sidebar-color',
                segments.includes('attach-file')
                  ? 'sidebar-color'
                  : 'sidebar-color/70',
                !isSidebarOpen ? 'justify-center' : ''
              )}
            >
              <Paperclip
                className="transition-transform duration-150 sidebar-color"
                style={{ fontSize: !isSidebarOpen ? '10rem' : '1.5rem' }} // Applying inline style for size
              />
              {isSidebarOpen && (
                <span className="text-sm font-medium sidebar-color">Attach File</span>
              )}
              <input
                id="file-input"
                type="file"
        ...
        <div
          ...
          style={{ right: isSidebarOpen ? '55px' : '2px' }} // Adjusted right position for left movement
        >
          <div
