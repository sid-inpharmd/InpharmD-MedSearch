...
const DashboardPage = () => {
      ...
      const uploadFilesToS3 = async () => {
        try {
          ...
          for (const file of files) {
            ...
            setFileData((prev) =>
              new Map(prev).set(files.indexOf(file), {
                summaryTableContent: '',
            ...
            await Promise.all([
              fetchSummaryTable(pdfUrl, files.indexOf(file)),
              fetchPdfTables(pdfUrl, files.indexOf(file)),
              fetchOverallSummary(pdfUrl, files.indexOf(file)),
            ]);
          }
  ...
  const fetchPdfTables = async (pdfUrl: string, index: number) => {
    ...
    try {
      ...
      setFileData(prev => new Map(prev).set(index, {
        ...prev.get(index),
        pdfTablesContent: 'Error fetching PDF tables',
      }));
    } finally {
              ...
              {activeTab === 'Summary Table' && (
                loadingStates.get(currentIndex)?.summaryTable ? (
                  <div className="flex flex-col items-center justify-center h-full">
                    ...
                    <div className="w-12 h-12 border-4 border-t-4 border-white border-t-orange-500 border-solid rounded-full animate-spin"></div>
                  </div>
                ) : (
                  <div
